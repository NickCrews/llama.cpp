...
#
# Example usage:
#   curl -s https://raw.githubusercontent.com/ggerganov/llama.cpp/master/download-llama-cpp.sh | bash -s -- /path/to/dest --tag=latest
...
get_tag() {
    ...
    TAG=$(
        curl -s "https://api.github.com/repos/ggerganov/llama.cpp/releases/latest" |
            sed -n 's/.*"html_url":"https:\/\/github.com\/ggerganov\/llama.cpp\/releases\/tag\/\([^"]*\)".*/\1/p'
    )
